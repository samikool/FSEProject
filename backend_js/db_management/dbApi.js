...
async ReturnDisaster(location){
  const query_str = `SELECT * from disasters WHERE location='${JSON.stringify(location)}';`;
  let res;
  try{
    res = await this.pool.query(query_str);
    res = res.rows
...
AddItem(name,type,keywords){
  const queryformat_keywords = keywords.length === 0 ? '' : '"' + keywords.join('","') + '"';
  const query_str = `insert into items (name,type,keywords)
  VALUES(
      '${name}',
      '${type}',
...
async AddDisaster(name,location,keywords){
   ...
   let query_str = `SELECT keywords FROM disasters where location='${JSON.stringify(location)}';`;
...
async VerifyUser(email,pword){
  let query_str = `SELECT PASSWORD FROM USERS where email='${email}'`;
  let golden_pw_enc;
 ...
 * @param isReq
 */
async NewUser(fname, lname, pword,
  email,location,isAdmin,
  isDon,isReq) {

  const enc_pword = await bcrypt.hash(pword, 10);
  let user_id;
  let res;
  let query_str = `INSERT INTO Users (First_Name, Last_Name, Email, Password, Location, isadmin,isDonor,isRequester)
  VALUES( '${fname}',
          '${lname}',
          '${email}',
          '${enc_pword}',
...
async DropUser(email){
  const query_str = `DELETE FROM USERS WHERE EMAIL = '${email}'`;
  let res = await this.pool.query(query_str);
}
...
async StoreToken(email, token){
  ...
  const query_str = `UPDATE USERS SET TOKEN = '${token}' WHERE EMAIL = '${email}'`;
  let res = await this.pool.query(query_str);
}
...
async ValidateToken(email, token){
  ...
  const query_str = `SELECT TOKEN FROM USERS WHERE EMAIL = '${email}'`;
  let res = await this.pool.query(query_str);
...
async RemoveToken(email, token){
  const query_str = `UPDATE USERS SET TOKEN = '' WHERE EMAIL = '${email}'`;
